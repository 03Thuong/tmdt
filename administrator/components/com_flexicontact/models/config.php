...
// set defaults for all our parameters so that we have this all in one place
	...
	if (!isset($this->data->button_text))      $this->data->button_text      = "";
...
function check($function, $param1)
{
	...
	switch ($param1)
		{
		case 'user_template':
			if (empty($this->data->user_subject))
                $errors[] = JText::_('COM_FLEXICONTACT_CANNOT_BE_BLANK').': '.JText::_('COM_FLEXICONTACT_USER_SUBJECT');	
            $result = JMailHelper::cleanSubject(JMailHelper::cleanLine($this->data->user_subject));
            if ($result != trim($this->data->user_subject))
                $errors[] = JText::_('COM_FLEXICONTACT_INVALID_EMAIL_TEXT');
			if (empty($this->data->user_template))
                $errors[] = JText::_('COM_FLEXICONTACT_CANNOT_BE_BLANK').': '.JText::_('COM_FLEXICONTACT_EMAIL_TEXT');
			if (strstr($this->data->user_template, LAFC_T_MESSAGE_DATA))			// %V_MESSAGE_DATA%
                $warnings[] = JText::_('COM_FLEXICONTACT_USER_DATA_IN_MESSAGE');
            $result = JMailHelper::cleanBody(JMailHelper::cleanText($this->data->user_template));
            if ($result != trim($this->data->user_template))
                $errors[] = JText::_('COM_FLEXICONTACT_INVALID_EMAIL_TEXT');
            break;

        case 'admin_template':
			if (empty($this->data->admin_subject))
                $errors[] = JText::_('COM_FLEXICONTACT_CANNOT_BE_BLANK').': '.JText::_('COM_FLEXICONTACT_ADMIN_SUBJECT');	
            $result = JMailHelper::cleanSubject(JMailHelper::cleanLine($this->data->admin_subject));
            if ($result != trim($this->data->admin_subject))
                $errors[] = JText::_('COM_FLEXICONTACT_INVALID_EMAIL_TEXT');
			if (empty($this->data->admin_template))
                $errors[] = JText::_('COM_FLEXICONTACT_CANNOT_BE_BLANK').': '.JText::_('COM_FLEXICONTACT_EMAIL_TEXT');
            $result = JMailHelper::cleanBody(JMailHelper::cleanText($this->data->admin_template));
            if ($result != trim($this->data->admin_template))
                $errors[] = JText::_('COM_FLEXICONTACT_INVALID_EMAIL_TEXT');
            break;
