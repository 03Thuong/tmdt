...
// Set up the configuration links
	...
	$config_table = array(
		...
		array(
			'link' => LAFC_COMPONENT_LINK.'&task=config&function=edit_form&param1=admin_template',
			'icon' => 'config_email_a.png',
			'name' => 'COM_FLEXICONTACT_EMAIL_TO_ADMIN',
			'desc' => 'COM_FLEXICONTACT_CONFIG_ADMIN_EMAIL_DESC'),
		array(
			'link' => LAFC_COMPONENT_LINK.'&task=config&function=edit_form&param1=user_template',
			'icon' => 'config_email_u.png',
			'name' => 'COM_FLEXICONTACT_EMAIL_TO_USER',
			'desc' => 'COM_FLEXICONTACT_CONFIG_USER_EMAIL_DESC'),
		array(
			'link' => LAFC_COMPONENT_LINK.'&task=config&function=edit_form&param1=confirm_text',
...
// any tabs?
	...
	foreach ($field_sets as $fieldset_name => $fieldset)
        {
        if (strstr($fieldset->class,'lad-fieldset-tab'))
            $form_has_tabs = true;        
        }
    if ($form_has_tabs)
		{
        ...
        echo JHtml::_(LAFC_TAB_UI.'.startTabSet','config_tabs', array('active' => 'main', 'recall' => true));
		}
	foreach ($field_sets as $fieldset_name => $fieldset)
        {
		if (strstr($fieldset->class,'lad-fieldset-tab'))
			{
			echo JHtml::_(LAFC_TAB_UI.'.addTab', 'config_tabs', $fieldset_name, JText::_($fieldset->label));
			switch ($fieldset_name)
				...
				case 'variables': 
					...
					break;
				case 'opt_fields':
					echo $this->render_opt_fields($form);
					break;
				default:
...
//-------------------------------------------------------------------------------
// Render the config fields for the one line text fields
//
function render_opt_fields($form)
{
	$html = '<table class="fc_field_table"><thead>';
	$html .= '<tr><td colspan="2">'.JText::_('COM_FLEXICONTACT_V_TEXT_FIELD_DESC').'</td><td>'.JText::_('COM_FLEXICONTACT_V_PROMPT').'</td></tr>';
	$html .= '</thead><tbody>';
	$html .= '<tr><td>'.JText::_('COM_FLEXICONTACT_FIELD_1').'</td><td>'.$form->getInput('field_opt1').'</td><td>'.$form->getInput('field_prompt1').'</td></tr>';
	$html .= '<tr><td>'.JText::_('COM_FLEXICONTACT_FIELD_2').'</td><td>'.$form->getInput('field_opt2').'</td><td>'.$form->getInput('field_prompt2').'</td></tr>';
	$html .= '<tr><td>'.JText::_('COM_FLEXICONTACT_FIELD_3').'</td><td>'.$form->getInput('field_opt3').'</td><td>'.$form->getInput('field_prompt3').'</td></tr>';
	$html .= '<tr><td>'.JText::_('COM_FLEXICONTACT_FIELD_4').'</td><td>'.$form->getInput('field_opt4').'</td><td>'.$form->getInput('field_prompt4').'</td></tr>';
	$html .= '<tr><td>'.JText::_('COM_FLEXICONTACT_FIELD_5').'</td><td>'.$form->getInput('field_opt5').'</td><td>'.$form->getInput('field_prompt5').'</td></tr>';
	$html .= '</tbody></table>';
	return $html;
}
...
function make_key_table()
{
	...
	$html = '<h5>'.JText::_('COM_FLEXICONTACT_CLICK_TO_COPY').'</h5>';
...
function edit_css()
{
	...
	if ($default_css_contents)
		...
		echo '<div class="lad-border-pad"><pre>'.$default_css_contents.'</pre></div>';
	    echo JHtml::_(LAFC_TAB_UI.'.endTab');
	    echo JHtml::_(LAFC_TAB_UI.'.endTabSet');
		}
